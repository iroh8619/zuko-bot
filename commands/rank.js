...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!score) {
        return interaction.editReply({ content: `${target.displayName || target.user.username} has no XP yet!` });
      }

      const top = sql.prepare("SELECT user, totalXP FROM levels WHERE guild = ? ORDER BY totalXP DESC")
